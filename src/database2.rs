...
impl Database {
    pub fn init(
        ...
        let database = if let Some(path) = path_option {
            ...
            sled::open(path).map_err(DbErrorr::DbStartError)?
        } else {
            // TODO
            /*
            tracing::warn!(
            ...
            sled::open("temp.db").map_err(DbErrorr::DbStartError)?
        };
        let orders = database.open_tree(ORDERS).map_err(DbErrorr::DbStartError)?;
    ...
    pub async fn order_list(&self) -> Result<Vec<(String, OrderInfo)>, DbErrorr> {
        let (res, rx) = oneshot::channel();
        let _unused = self.tx.send(DbRequest::ActiveOrderList(res)).await;
        rx.await.map_err(|_| DbErrorr::DbEngineDown)?
    }
    ...
    ) -> Result<OrderCreateResponse, DbErrorr> {
        let (res, rx) = oneshot::channel();
        let _unused = self
            .tx
    ...
    pub async fn read_order(&self, order: String) -> Result<Option<OrderInfo>, DbErrorr> {
        let (res, rx) = oneshot::channel();
        let _unused = self
            .tx
    ...
    pub async fn mark_paid(&self, order: String) -> Result<OrderInfo, DbErrorr> {
        let (res, rx) = oneshot::channel();
        let _unused = self
            .tx
    ...
    pub async fn mark_withdrawn(&self, order: String) -> Result<(), DbErrorr> {
        let (res, rx) = oneshot::channel();
        let _unused = self
            .tx
    ...
    pub async fn mark_stuck(&self, order: String) -> Result<(), DbErrorr> {
        let (res, rx) = oneshot::channel();
        let _unused = self
            .tx
...
enum DbRequest {
    ...
    ActiveOrderList(oneshot::Sender<Result<Vec<(String, OrderInfo)>, DbErrorr>>),
    ReadOrder(ReadOrder),
    MarkPaid(MarkPaid),
...
pub struct CreateOrder {
    ...
    pub res: oneshot::Sender<Result<OrderCreateResponse, DbErrorr>>,
}

pub struct ReadOrder {
    pub order: String,
    pub res: oneshot::Sender<Result<Option<OrderInfo>, DbErrorr>>,
}

pub struct ModifyOrder {
    pub order: String,
    pub res: oneshot::Sender<Result<(), DbErrorr>>,
}

pub struct MarkPaid {
    pub order: String,
    pub res: oneshot::Sender<Result<OrderInfo, DbErrorr>>,
}
...
fn read_order(order: String, orders: &sled::Tree) -> Result<Option<OrderInfo>, DbErrorr> {
...
fn mark_paid(order: String, orders: &sled::Tree) -> Result<OrderInfo, DbErrorr> {
    if let Some(order_info) = orders.get(order.clone())? {
        ...
        if order_info.payment_status == PaymentStatus::Pending {
            ...
            Err(DbErrorr::AlreadyPaid(order))
        }
    } else {
        Err(DbErrorr::OrderNotFound(order))
    }
}
fn mark_withdrawn(order: String, orders: &sled::Tree) -> Result<(), DbErrorr> {
    if let Some(order_info) = orders.get(order.clone())? {
        let mut order_info = OrderInfo::decode(&mut &order_info[..])?;
        if order_info.payment_status == PaymentStatus::Paid {
            if order_info.withdrawal_status == WithdrawalStatus::Waiting {
                ...
                Err(DbErrorr::WithdrawalWasAttempted(order))
            }
        } else {
            Err(DbErrorr::NotPaid(order))
        }
    } else {
        Err(DbErrorr::OrderNotFound(order))
    }
}
fn mark_stuck(order: String, orders: &sled::Tree) -> Result<(), DbErrorr> {
    if let Some(order_info) = orders.get(order.clone())? {
        let mut order_info = OrderInfo::decode(&mut &order_info[..])?;
        if order_info.payment_status == PaymentStatus::Paid {
            if order_info.withdrawal_status == WithdrawalStatus::Waiting {
