use crate::{
    AssetId, Balance, BlockHash, BlockNumber, Chain, Decimals, OnlineClient, PalletIndex,
    RuntimeConfig,
};
...
use subxt::{
    ...
    dynamic::{self, At},
    ext::{scale_decode::DecodeAsType, sp_core::crypto::Ss58AddressFormat},
    storage::Storage,
};
...
pub struct ChainProperties {
    ...
    pub assets: HashMap<AssetId, AssetProperties>,
...
impl ChainProperties {
    async fn fetch(
        ...
        let ex_dep: u128 = fetch_constant(constants, EXISTENTIAL_DEPOSIT)?;
        let mut assets_props = HashMap::new();

        for asset in assets {
            assets_props.insert(
                asset,
                AssetProperties {
                    min_balance: fetch_min_balance(storage_finalized.clone(), asset).await?,
                    decimals: fetch_decimals(storage_finalized.clone(), asset).await?,
                },
            );
        }
        ...
        Ok(Self {
            ...
            existential_deposit: Balance(ex_dep),
            assets: assets_props,
...
pub async fn prepare(chains: Vec<Chain>) -> Result<HashMap<String, ConnectedChain>> {
    let mut connected_chains = HashMap::with_capacity(chains.len());

    for chain in chains {
        let endpoint = chain.endpoints.first().with_context(|| {
            format!(
                "{:?} chain doesn't have any `endpoints` in the config",
                chain.name
            )
        })?;
        let rpc_client = RpcClient::new(
            Client::builder()
                ...
                .with_context(|| {
                    format!(
                        "failed to construct the RPC client for the {:?} chain",
                        chain.name
                    )
                })?,
        );
        ...
        log::info!(
            "Connected to an RPC server for the {:?} chain at {endpoint:?}.",
            chain.name
        );

        let methods = LegacyRpcMethods::new(rpc_client.clone());
        let client = OnlineClient::from_rpc_client(rpc_client)
            .await
            .with_context(|| {
                format!(
                    "failed to construct the API client for the {:?} chain",
                    chain.name
                )
            })?;
        ...
        let connected_chain = ConnectedChain {
            methods,
            genesis: client.genesis_hash(),
            assets: client
                .metadata()
                .pallet_by_name(ASSETS)
                .map(|pallet| pallet.index()),
            properties: ChainProperties::fetch(
                &client.constants(),
                client.storage().at_latest().await?,
                chain
                    .asset
        ...
        if connected_chains
            .insert(chain.name.clone(), connected_chain)
            .is_some()
        {
            anyhow::bail!(
                "found `[chain]`s in the config with the same name ({:?}), all chain names must be unique",
                chain.name
            );
        }
    }
